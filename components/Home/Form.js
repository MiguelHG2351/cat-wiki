    ...
    <>
      <form>
        <div className="relative inline-block">
          ...
          <label htmlFor={ inputId } onClick={ toggleMenuMobile } type="button" className="inline-block w-[16ch] text-left text-black bg-white rounded-3xl py-2 px-3 outline-none placeholder:text-black lg:hidden">
            Search
          </label>
        ...
        <div style={ { display: menu ? 'block' : 'none' } } className="modal-mobile fixed z-20 top-0 left-0 right-0 bottom-0 py-2 px-4 bg-white">
          ...
          <div className="input-mobile-container group relative">
            <Input search={search} id={ inputId } onInput={ onInputHandler } className=" border-black border w-full my-7 rounded-lg py-2 px-3 placeholder:text-black" type="text" name="search-modal" placeholder="Search for a cat" />
          </div>
        </div>
      </form>
    </>
